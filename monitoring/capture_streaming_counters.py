...
the host creates --fourth-trigger-file following its container start. Once
node 4 has answered, a missing response is an error, not a silent gap.
"""
...
FIELDS = ('timestamp_utc', 'sample', 'node', 'node_count', 'shard', 'direction', 'bytes')
...
def read_counters(node: str) -> dict[tuple[str, str], int]:
    ...
    if not counters:
        raise ValueError(f'{node}: no streaming-byte counters in metrics response')
    shards = {shard for shard, _ in counters}
    if set(counters) != {(shard, direction) for shard in shards for direction in ('incoming', 'outgoing')}:
        raise ValueError(f'{node}: missing incoming or outgoing counter for a shard')
    return counters
...
def main() -> None:
    ...
    if args.interval_seconds <= 0 or (args.duration_seconds is not None and args.duration_seconds <= 0):
    ...
    try:
        with args.output.open('x', newline='', encoding='utf-8') as output:
            ...
            while True:
                ...
                readings = {}
                for node in NODES:
                    if node == 'r-scylla4' and not (fourth_seen or args.fourth_trigger_file.exists()):
                        continue
                    try:
                        readings[node] = read_counters(node)
                    except OSError:
                ...
                for node, counters in readings.items():
                    for (shard, direction), count in sorted(counters.items()):
                        writer.writerow(dict(timestamp_utc=timestamp, sample=sample,
                                             node=node, node_count=node_count, shard=shard,
                                             direction=direction, bytes=count))
                output.flush()
                if sample == 0 or node_count != previous_node_count:
                    print(f'Sample {sample}: {node_count} nodes at {timestamp}', flush=True)
                previous_node_count = node_count
                sample += 1
                if args.duration_seconds is not None and time.monotonic() - started >= args.duration_seconds:
                    break
                time.sleep(max(0, args.interval_seconds - (time.monotonic() - cycle_started)))
