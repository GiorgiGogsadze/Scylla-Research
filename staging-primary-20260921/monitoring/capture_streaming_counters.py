"""Sample per-shard streaming counters through a 3-to-4-to-3 lifecycle.
...
any missing response or counter is an error until a separately timestamped
--departure-trigger-file marks the start of graceful decommission. After that
marker, node 4 may disappear once; nodes 1-3 remain mandatory. Preserve the
last node-4 sample because its final counter delta may be unobservable.
"""
...
def main() -> None:
    ...
    parser.add_argument('--departure-trigger-file', type=Path, required=True)
        ...
        with args.output.open('x', newline='', encoding='utf-8') as output:
            ...
            while True:
                ...
                for node in NODES:
                    ...
                    if node == 'r-scylla4' and (fourth_departed or not (fourth_seen or fourth_triggered)):
                        continue
                    try:
                        counters = read_counters(node)
                        counter_keys = set(counters)
                        if baseline_keys is None:
                            baseline_keys = counter_keys
                        elif counter_keys != baseline_keys:
                            if node == 'r-scylla4' and not fourth_seen:
                                raise CountersNotReady(
                                    f'{node}: shard/direction keys differ from baseline'
                                )
                            raise ValueError(f'{node}: shard/direction keys changed')
                        ...
                        for key, value in counters.items():
                            previous = previous_counters.get((node, key))
                            if previous is not None and value < previous:
                                raise ValueError(
                                    f'{node}: streaming counter decreased for {key}'
                                )
                        ...
                        if (
                            node == 'r-scylla4'
                            and args.departure_trigger_file.exists()
                            and fourth_seen
                        ):
                            fourth_departed = True
                            print(
                                f'Node 4 metrics disappeared during marked departure '
                                f'at {timestamp}: {error}', flush=True,
                            )
                            continue
                        raise
                ...
                for node, counters in readings.items():
                    previous_counters.update(
                        { (node, key): value for key, value in counters.items() }
                    )
