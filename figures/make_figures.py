...
ROOT = Path(__file__).resolve().parent / 'figure-data'
...
for ax, operation in zip(axes, ['read', 'write']):
    ...
    ax.set_title(f'{operation.capitalize()} p95 client operation latency')
    ax.set_xticks(x, LABELS, rotation=25, ha='right')
    ax.set_ylabel('Latency (ms)')
    ax.grid(axis='y', alpha=.25)
axes[0].legend(frameon=False)
fig.suptitle('Client operation latency tails remained in the millisecond range')
...
save(fig, 'figure_4_phase_scheduling_lag')
...
save(fig, 'figure_5_transition_lag_timeline')
...
ax.set_title('Placement-and-counter quiescence: retrospective onset')
ax.grid(axis='y', alpha=.25)
ax.legend(frameon=False)
save(fig, 'figure_2_convergence_onset')
...
save(fig, 'figure_3_movement_observability')
